...
RSpec.describe RuboCop::Cop::Angellist::ForbidInstanceVariableGetSet, :config do
  let(:config) { RuboCop::Config.new('Angellist/ForbidInstanceVariableGetSet' => { 'Enabled' => true }) }

  it 'registers an offense when using instance_variable_get' do
    expect_offense(<<~RUBY)
      object.instance_variable_get(:@foo)
      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Avoid using `instance_variable_get` or `instance_variable_set`. Prefer using public getters/setters.
    RUBY
  end
