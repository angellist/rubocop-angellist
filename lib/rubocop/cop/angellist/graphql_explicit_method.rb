    ...
    module Angellist
      ...
      class GraphqlExplicitMethod < Base
        ...
        MSG = 'GraphQL field `:%<field_name>s` has no explicit `method:` or ' \
              '`resolver_method:` option. Add `method: :%<method_name>s` (if delegating to the data object) ' \
              'or `resolver_method: :%<method_name>s` (if a custom def exists in this class).'
        ...
        def on_send(node)
          ...
          message = format(MSG, field_name: name, method_name: method_name)
